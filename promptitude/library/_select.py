...
async def select(
    variable_name: str = "selected",
    options: Union[List[str], None] = None,
    logprobs: Union[str, None] = None,
    list_append: bool = False,
    llm_alt_model: Union[str, llms.LLM, None] = None,
    _parser_context: Union[Dict[str, Any], None] = None
) -> Any:
    """
    ...
    variable_name : str, optional
        The name of the variable to set with the selected value. Default is "selected".
    options : list of str or None, optional
        ...
        Default is None.
    logprobs : str or None, optional
        An optional variable name to set with the log probabilities for each option. If this is set, the log probabilities
        of every option are fully evaluated. When this is None (the default), a greedy max approach is used to select the
        option (similar to how greedy decoding works in a language model). In some cases, the selected option can change
        when `logprobs` is set, since it will be more like an exhaustive beam search scoring than a greedy max scoring.
    list_append : bool, optional
        Whether to append the generated value to a list stored in the variable. If set to True, the variable
        must be a list, and the generated value will be appended to the list. Default is False.
    llm_alt_model : str or llms.LLM or None, optional
        An alternative LLM model to use for this selection. If provided, this LLM will be used instead of the default one.
        Default is None.
    ...
    -------
    Any
        The selected value(s) after selection.

    Raises
    ------
    ValueError
        If required arguments are missing or if no valid option is generated.
    ...
    >>> program = guidance("Is Everest very tall? Answer 'Yes' or 'No': '{{#select 'name'}}Yes{{or}}No{{/select}}")
    >>> output = program()
    >>> print(output["name"])

    Use with specified options:

    >>> program = guidance("They work at: {{select 'company' options=companies}}")
    >>> output = program(companies=["Microsoft", "Apple", "Google"])
    >>> print(output["company"])

    Note
    ----
    The `select` command allows you to choose a value from a list of options based on the LLM's prediction.
    """
    ...
    try:
        if block_content is None:
            if options is None:
                raise ValueError(
                    "You must provide an options list when using the select command in non-block mode. "
                    "Example: {{select 'variable_name' options=options_list}}"
                )
        else:
            if len(block_content) <= 1:
                raise ValueError("You must provide at least two options to the select block command.")
            if options is not None:
                raise ValueError("You cannot provide an options list when using the select command in block mode.")
        ...
        # Ensure the original LLM and session are restored
        if llm_alt_model is not None:
